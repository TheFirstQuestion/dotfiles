...
GitHub: https://github.com/cdown/tzupdate
"""
...
SERVICES = frozenset(
    [
        ...
        GeoIPService("https://ipapi.co/{ip}/json/",
                     ("timezone",), ("reason",)),
        GeoIPService(
            "http://worldtimeapi.org/api/ip/{ip}", ("timezone",), ("error",)),
    ]
)
...
def get_timezone(ip_addr, timeout=DEFAULT_HTTP_TIMEOUT, services=SERVICES):
    ...
    threads = [
        Process(target=get_timezone_for_ip, args=(
            ip_addr, svc, api_resp_queue))
        for svc in services
    ]
...
def get_sys_timezone(zoneinfo_abspath, localtime_abspath):
    return localtime_abspath.replace(
        os.path.commonprefix(
            [zoneinfo_abspath, localtime_abspath]) + os.path.sep, "", 1
    )
...
def main(argv=None, services=SERVICES):
    ...
    if args.timezone:
        ...
        timezone = get_timezone(
            args.ip, timeout=args.timeout, services=services)
